...
interface PaymentSourceManagerProps {
  ...
  onAddSource: (name: string) => Promise<string | null>;
  onEditSource: (source: PaymentSource) => Promise<void>;
  onDeleteSource: (id: string) => Promise<void>;
}

export const PaymentSourceManager: React.FC<PaymentSourceManagerProps> = ({
  sources,
  onAddSource,
  onEditSource,
  onDeleteSource
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!trimmed) return;
    const id = await onAddSource(trimmed);
    if (id) setNewName('');
  };
  ...
  const saveEdit = async (e: React.FormEvent) => {
    ...
    if (!trimmed) return;
    await onEditSource({ id: editingId, name: trimmed });
    cancelEdit();
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-800">Ödeme Kaynağı Listesi</h2>
        <p className="text-gray-500">Ödeme kaynağı adı ekleme, düzenleme ve silme</p>
      </div>
      ...
      {confirmDeleteId && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full shadow-xl">
            <p className="text-gray-800 font-medium mb-4">Bu ödeme kaynağını silmek istiyor musun?</p>
            <div className="flex gap-3 justify-end">
              ...
              <button
                type="button"
                onClick={async () => {
                  await onDeleteSource(confirmDeleteId);
